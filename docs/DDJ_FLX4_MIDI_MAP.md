...
| Play + Shift / Censor | `0x90/0x0E`, `0x91/0x0E` | press/release | shifted deck-local | `CTRL_ID_DECK*_EXT_ACTION` / `CTRL_DECK_EXT_ACTION_CENSOR` | `deck_core` slip-censor MVP | Implemented MVP: press seeks about 1 s back and release rejoins the advancing timeline; not true reverse | Host-tested from XML; D1 state, LED and audible repeat hardware-confirmed 2026-08-24; each seek added one output-late event and 256 D1 PCM-underrun frames, while a control start/stop added none |
