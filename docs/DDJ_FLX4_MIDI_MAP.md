...
| Beat Jump pads 1-8 | D1 `0x97/0x20..0x27`, D2 `0x99/0x20..0x27` | press/release; default pad sizes `−1`, `+1`, `−2`, `+2`, `−4`, `+4`, `−8`, `+8` beats | active Beat Jump mode | `CTRL_ID_DECK1_PAD_ACTION`, `CTRL_ID_DECK2_PAD_ACTION` with mode+pad | `deck_core` beat jump using the global size page | Implemented | Input addresses verified D1 pads 1-8 and D2 pads 1,3-8 2026-06-21; earlier hardware behavior smoke predates corrected XML size order; corrected order host-tested 2026-08-24 and requires hardware reconfirmation |
| Beat Jump shifted size pads 7/8 | D1 `0x98/0x26..0x27`, D2 `0x9A/0x26..0x27` | press changes one global size page, release ignored; pad 7 decreases and pad 8 increases by factor 16 with saturation | shifted Beat Jump mode | same pad action ID with shift flag | P4 `deck_core` global page: fractional `±1/16..±1/2`, default `±1..±8`, large `±16..±128` | Implemented | XML/Mixxx behavior and both-deck shared state host-tested 2026-08-24; hardware smoke pending |
...
| Beat Jump pad LEDs | normal D1 `0x97/0x20..0x27`, normal D2 `0x99/0x20..0x27`; shifted mirror for pads 1-6 D1 `0x98/0x20..0x25`, D2 `0x9A/0x20..0x25` | track loaded while Beat Jump mode is selected | P4 `deck_core` publishes normal pad LED diff, shifted pads 1-6 mirror and reconnect refresh | Implemented normal + shifted mirror output for pads 1-6; shifted pads 7/8 are explicit page helpers | Packet mapping, state and reconnect capacity host-tested 2026-08-24; normal LEDs passed hardware smoke 2026-07-07, shifted mirror hardware smoke pending |
| Beat Jump shifted helper LEDs | D1 `0x98/0x26..0x27`, D2 `0x9A/0x26..0x27` | while track is loaded, Beat Jump mode is selected and deck Shift is held: pad 7 is ON if a lower page exists, pad 8 if a higher page exists | P4 `deck_core` global Beat Jump page and per-deck Shift state | Implemented boundary-aware shifted helper output and reconnect refresh | Packet mapping, default/boundary state and both-deck global page host-tested 2026-08-24; boundary behavior hardware smoke pending |
