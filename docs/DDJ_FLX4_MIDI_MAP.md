...
| Play + Shift / Censor | `0x90/0x0E`, `0x91/0x0E` | press/release | shifted deck-local | `CTRL_ID_DECK*_EXT_ACTION` / `CTRL_DECK_EXT_ACTION_CENSOR` | `deck_core` + `audio_censor` gapless slip-reverse | Implemented: reverse reads bounded canonical PCM history while the forward playhead advances; release uses a 10-ms crossfade and no seek | XML/host-tested, including mixed-rate interpolation, bounded edge and no-seek deck contract; previous D1 seek-MVP state/LED/repeat was hardware-confirmed 2026-08-24, new DSP hardware smoke pending |
