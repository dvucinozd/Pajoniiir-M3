...
| Browse + Shift rotate | `0xB6/0x64` | relative encoder | shifted global | `CTRL_ID_BROWSE_SHIFT_DELTA` | UI Library / Overview accelerated navigation | Implemented | XML/host-tested; Library eyes-on hardware smoke passed 2026-09-01 |
| Browse + Shift press | `0x96/0x42` | press/release | shifted global | `CTRL_ID_BROWSE_SHIFT_PRESS` | UI Library force-open | Implemented | XML/host-tested; force-open hardware smoke passed 2026-09-01 |
...
| Shift + Load Deck 1 / Deck 2 | `0x96/0x68`, `0x96/0x7A` | press/release | shifted global, deck from midino | `CTRL_ID_SHIFT_LOAD_DECK1`, `CTRL_ID_SHIFT_LOAD_DECK2` | UI Library | Implemented; currently same load behavior as normal Load with distinct observability IDs | Official PDF/XML and host-tested; D1/D2 routing hardware smoke passed 2026-09-01 |
...
| Physical control | XML status/midino | Encoding | Deck/shift | Semantic ID | P4 owner | Status | HW verification |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Hot Cue pads 1-8 | D1 `0x97/0x00..0x07`, D2 `0x99/0x00..0x07` | press/release | active Hot Cue mode | `CTRL_ID_DECK1_PAD_ACTION`, `CTRL_ID_DECK2_PAD_ACTION` with mode+pad | P4 hot cue state | Implemented behavior + live UI overlay | Verified D1/D2 pads 1-8 2026-06-21; D1 A set/recall, immediate UI refresh and post-reboot NVS reload accepted 2026-09-01 |
| Hot Cue clear pads 1-8 | D1 `0x98/0x00..0x07`, D2 `0x9A/0x00..0x07` | press/release | shifted Hot Cue mode | same pad action ID with shift flag | P4 hot cue state | Implemented behavior + live UI overlay | D1 shifted clear and immediate EMPTY UI accepted 2026-09-01; D2 shifted clear behavior smoke pass 2026-06-26 |
...
| Hot Cue pad LEDs | normal D1 `0x97/0x00..0x07`, normal D2 `0x99/0x00..0x07`; shifted mirror D1 `0x98/0x00..0x07`, D2 `0x9A/0x00..0x07` | hot cue slot exists while Hot Cue mode is selected | P4 hot cue store state | Implemented normal + shifted mirror output | Packet ranges and reconnect capacity host-tested; shifted hardware smoke passed on both decks 2026-08-24; D1 A store/clear/reload LED/UI owner path accepted 2026-09-01 |
